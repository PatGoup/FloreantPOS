...
public class CustomerBookingForm extends BeanEditor<Customer> {
	...
	public CustomerBookingForm(boolean enable) {
		isKeypad = enable;
		createCustomerForm();
	}
	...
	private void createCustomerForm() {
		...
		JLabel lblState = new JLabel(Messages.getString("CustomerBookingForm.0")); //$NON-NLS-1$
	...
	protected boolean updateModel() throws IllegalModelStateException {
		...
		if (StringUtils.isEmpty(mobile) && StringUtils.isEmpty(name)) {
			POSMessageDialog.showError(null, Messages.getString("CustomerBookingForm.1")); //$NON-NLS-1$
			return false;
		}
		Customer customer = (Customer) getBean();
