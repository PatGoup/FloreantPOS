...
public abstract class SelectionViewWL extends JPanel implements ComponentListener {
...
//???	protected CardLayout cardLayout = new CardLayout();        //???
	protected WrapLayout cardLayout = new WrapLayout();        //???	
//???    protected FlowLayout cardLayout = new FlowLayout(FlowLayout.CENTER);        //???	
//???	private JPanel cardLayoutContainer = new JPanel(cardLayout);
    private JPanel cardLayoutContainer = new JPanel(new GridLayout(0,1));      //???
 
//???	protected JPanel buttonPanelContainer = new JPanel(new BorderLayout());
	protected JPanel buttonPanelContainer = new JPanel(new GridLayout(0,1));      //???
...
//???	
	protected Color colorActiveButton = null;;
	protected Color clrBackground;
	protected JToggleButton btnActive;
...
//???		
	...
	public SelectionViewWL (String title, int buttonWidth, int buttonHeight) {
		...
		//???
        /*********************************88
		border = new TitledBorder ("CardLayoutContainer");
        border.setTitleJustification(TitledBorder.CENTER); 
        cardLayoutContainer.setBorder (border);
       	
		border = new TitledBorder ("buttonPanelContainer");
        border.setTitleJustification(TitledBorder.CENTER);
		 buttonPanelContainer.setBorder (border);
		************************/
		//???
		
		add(buttonPanelContainer);		
		buttonPanelContainer.add (cardLayoutContainer);
        cardLayoutContainer.revalidate ();    //???

        //???
        actionButtonPanel.setName ("actionbuttons");
        actionButtonPanel.setBorder (new EmptyBorder (0,0,6,6));
        //???
	...
	public void setActiveColor (Color colorActiveButton)
	{
        this.colorActiveButton = colorActiveButton;
    }
...
//???		return totalItem;
	...
	//???		ButtonPanel buttonPanel = null;
                ...
                if (j == 0) {
					...
					buttonPanel.setName (i + "  " + title);
					
			//		border = new TitledBorder ("buttonPanel");
              //      border.setTitleJustification(TitledBorder.CENTER);
             //       buttonPanel.setBorder (border);
					
					//???***   what what what
					//???***  YES  need to scroll the just created buttonPanel
                   cardLayoutContainer.add(buttonPanel, buttonPanel.getName());
           //???         JScrollPane scrollPane = new JScrollPane((JPanel)buttonPanel);
                    cardLayoutContainer.add(buttonPanel);
       //???             cardLayoutContainer.add(scrollPane);
				}
                j++;
                ...
                //??? add the button to the holder panel
                //???*** add it to gridbox scroller ????
